...
<?php
	...
	//free 테이블생성
	$flag = "NO";
	$sql = "show tables from classDB"; //클래스DB에 있는 테이블들을 다 보여줘라
	$result = mysqli_query($con, $sql) or die("실패원인:".mysqli_error($con)); //$result = 정보의 대한 주소를 가져온다
	
	while($row = mysqli_fetch_row($result)){ //result테이블의 결과값이 없을때까지
	    if($row[0]==="free"){
	        $flag = "OK";
	        break;
	    }
	...
	if($flag !== "OK"){
	    $sql = "create table free (
                    ...
                    primary key(num)
                )"; //제대로 실행 됬는지 안됐는지만 알면된다.(create insert delete update)
	    if(mysqli_query($con, $sql)){ //mysqli_query를 처리하고 결과만 가지고 있다
